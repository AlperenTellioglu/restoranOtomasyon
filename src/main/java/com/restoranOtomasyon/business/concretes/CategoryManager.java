...
public class CategoryManager implements CategoryService {
	...
	public List<GetAllCategoriesResponse> getAllCategories() {
		...
		for (Category category : categories) {
			...
			List<GetAllMenusResponse> menuResponseList = category.getMenus().stream().map(menu -> {
	...
	public List<GetAllCategoriesWithStatusResponse> getAllCategoriesWithStatus() {
		...
		List<GetAllCategoriesWithStatusResponse> categoryResponseList = new ArrayList<GetAllCategoriesWithStatusResponse>();

		for (Category category : categories) {
			GetAllCategoriesWithStatusResponse responseItem = new GetAllCategoriesWithStatusResponse();
			responseItem.setId(category.getCategoryId());
			responseItem.setName(category.getCategoryName());

			List<GetAllMenusResponse> menuResponseList = category.getMenus().stream()
					.filter(menu -> !"gosterme".equals(menu.getMenuStatus())).map(menu -> {
						GetAllMenusResponse menuResponse = new GetAllMenusResponse();
						menuResponse.setId(menu.getMenuId());
						menuResponse.setName(menu.getMenuName());
						menuResponse.setPrice(menu.getPrice());
